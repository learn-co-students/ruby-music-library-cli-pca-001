class MusicLibraryController
  ...
  def list_artists
    all_artists = Artist.all.sort_by(&:name)
  ...
  def list_genres
    all_genres = Genre.all.sort_by(&:name)
  ...
  def list_songs_by_artist
    ...
    if artist
      artist_songs = artist.songs.sort_by(&:name)
  ...
  def list_songs_by_genre
    ...
    if genre
      genres = genre.songs.sort_by(&:name)
  ...
  def play_song
    ...
    if selected_song && input.to_i.positive? && input.to_i < 6
